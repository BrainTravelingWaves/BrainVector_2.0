...
for k=1:length(name)
    ...
    seeg=zeros(size(OpMEEGbem129.Gain,1)-1,PARAM.N_step);
    ...
    tic   
    parfor i=1:N  
      [dist,path, pred]=graphshortestpath(Al,vert(i+N*(j-1))-ii,'Directed', false);
      ElemDip=cortex_edipl0(cor_l, dist, PARAM );
